...
import { Download, Users, Package, Receipt, Percent, AlertTriangle, MapPin, FileSpreadsheet } from 'lucide-react';
...
const mockEstimate: CostEstimate = {
  labourCost: 12500000,
  materialCost: 38500000,
  equipmentCost: 5200000,
  overheads: 2800000,
  profitMargin: 5900000,
  contingencies: 3200000,
  total: 68100000,
};
...
export function CostEstimation() {
  ...
  const adjustedTotal = mockEstimate.total * locationFactor;
    ...
    () => [
      {
        ...
        label: 'Labour',
        amount: mockEstimate.labourCost * locationFactor,
      },
      {
        key: 'materialCost' as const,
        label: 'Materials',
        amount: mockEstimate.materialCost * locationFactor,
      },
      {
        key: 'equipmentCost' as const,
        label: 'Equipment',
        amount: mockEstimate.equipmentCost * locationFactor,
      },
      {
        key: 'overheads' as const,
        label: 'Overheads',
        amount: mockEstimate.overheads * locationFactor,
      },
      {
        key: 'profitMargin' as const,
        label: 'Profit Margin',
        amount: mockEstimate.profitMargin * locationFactor,
      },
      {
        key: 'contingencies' as const,
        label: 'Contingencies',
        amount: mockEstimate.contingencies * locationFactor,
      },
    ],
    [locationFactor]
  );
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {summaryCards.map((card, i) => {
          ...
          const value = mockEstimate[card.key] * locationFactor;
          return (
            <motion.div
              key={card.key}
          ...
          <CardContent className="space-y-4">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {categories.map((cat) => {
                    const pct = (cat.amount / adjustedTotal) * 100;
                    return (
                      <tr
                        key={cat.key}
              ...
              <div className="mt-4 flex items-center gap-2 text-sm text-[var(--sys-on-surface-variant)] bg-[var(--sys-surface-container)] rounded-[var(--sys-corner-sm)] px-4 py-3">
                ...
                <span>
                  ...
                  <strong className="text-[var(--sys-on-surface)]">
                    {formatCurrency(mockEstimate.total)}
                  </strong>{' '}
                  → Adjusted{' '}
